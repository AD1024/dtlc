...
let rec alpha_equiv (e1 : Syntax.raw_expr) (e2 : Syntax.raw_expr) =
    match e1, e2 with
    | Var x1, Var x2 -> x1 = x2
    | App (e11, e12), App (e21, e22)                  -> alpha_equiv e11 e21 && alpha_equiv e12 e22
    | Lambda (x1, t1, e11), Lambda (x2, t2, e21)      -> alpha_equiv t1 t2 && alpha_equiv e11 (Syntax.subst x2 (Var x1) e21)
    | Pi ((x1, e11), e12), Pi ((x2, e21), e22)
    | Pair ((x1, e11), e12), Pair ((x2, e21), e22)
    | Sigma ((x1, e11), e12), Sigma ((x2, e21), e22)      -> alpha_equiv e11 e21 && alpha_equiv e12 (Syntax.subst x2 (Var x1) e22)
    | Fst e1, Fst e2 | Snd e1, Snd e2 | Succ e1, Succ e2  -> alpha_equiv e1 e2
    | PropEq (e11, e12), PropEq (e21, e22)                -> alpha_equiv e11 e21 && alpha_equiv e12 e22
    | NatElim (e11, e12, e13, e14), NatElim (e21, e22, e23, e24)  -> alpha_equiv e11 e21 && alpha_equiv e12 e22 && alpha_equiv e13 e23 && alpha_equiv e14 e24
    | EqElim (e11, e12, e13, e14), EqElim (e21, e22, e23, e24)    -> alpha_equiv e11 e21 && alpha_equiv e12 e22 && alpha_equiv e13 e23 && alpha_equiv e14 e24
    | Refl (e11, e12), Refl (e21, e22)                            -> alpha_equiv e11 e21 && alpha_equiv e12 e22
    | Type i1, Type i2 ->
...
let rec type_infer (gamma : Syntax.raw_expr Syntax.Gamma.t) (e : Syntax.raw_expr) =
  ...
  | Lambda (x, e1, e2) -> let gamma' = type_check gamma e1 (Syntax.Type None) in
                          let e3 = type_infer (Syntax.Gamma.add x e1 gamma') e2 in
                          Pi ((x, e1), e3)
  | App (e1, e2)       -> let ty_e1 = type_infer gamma e1 in
                          begin match ty_e1 with
                                | Pi ((x, e3), e4) -> let _ = type_check gamma e2 e3 in
  ...
  | Pi ((x, e1), e2) | Sigma ((x, e1), e2) ->
                          let gamma_1 = type_check gamma e1 (Syntax.Type None) in
                          let _ = type_check (Syntax.Gamma.add x e1 gamma_1) e2 (Syntax.Type None) in
                          Syntax.Type None
  | Pair ((x, e1), e2) -> let ty_e1 = type_infer gamma e1 in
                          let ty_e2 = type_infer (Syntax.Gamma.add x ty_e1 gamma) e2 in
                          Syntax.Sigma ((x, ty_e1), ty_e2)
  | Fst e     ->  let ty_e = type_infer gamma e in
                  begin match ty_e with
                        | Sigma ((_, e1), _) -> e1
                        | _ -> raise (TypeError (Printf.sprintf "Cannot apply %s with Fst" (Syntax.show_raw_expr e)))
                  end
  | Snd e     ->  let ty_e = type_infer gamma e in
                  begin match ty_e with
  ...
  | Zero | Succ _ ->  Nat
  | Refl (e1, e2) ->  let gamma' = type_check gamma e1 (Type None) in
                      let _      = type_check gamma' e2 e1 in
                      Syntax.PropEq (e2, e2)
  | PropEq (e1, e2) ->  let ty_e1 = type_infer gamma e1 in
                        let _ = type_check gamma e2 ty_e1 in
                        Syntax.Type None
  | NatElim (e1, e2, e3, e4) ->
                  let gamma_1 = type_check gamma e4 Syntax.Nat in
                  begin match e1 with
                        | Lambda (z, Nat, goal) ->
  ...
  | EqElim (e1, e2, e3, e4) ->
                    let gamma' = type_check gamma e1 (Syntax.Type None) in
                    let ty_e4 = type_infer gamma e4 in
                    begin match ty_e4 with
                      | PropEq (lhs, rhs) ->
                        let ty_lhs = type_infer gamma' lhs in
                        let gamma' = type_check gamma' rhs ty_lhs in
                        begin match e2 with
                              | Lambda (x, _, Lambda (y, _, goal)) ->
                                let gamma' = type_check (Syntax.Gamma.add y e1 (Syntax.Gamma.add x e1 gamma')) goal (Syntax.Type None) in
                                begin match e3 with
                                      | Lambda (w, _, e3) ->
                                        let _ = type_check (Syntax.Gamma.add w e1 gamma') e3 (Syntax.subst x (Syntax.Var w) (Syntax.subst y (Syntax.Var w) goal)) in
